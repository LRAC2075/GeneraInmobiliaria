    ...
    <>
      ...
      <div className="container mx-auto px-4 py-12">
        <SectionHeader 
          ...
          subtitle="Explore nuestra selección curada de propiedades de lujo. Cada una representa la cúspide del diseño, la comodidad y la ubicación privilegiada."
        />

        <section className="bg-light-card dark:bg-gray-900 rounded-lg p-8 md:p-12 mb-12">
          <h3 className="text-3xl font-bold text-center text-light-text dark:text-white mb-8">Nuestro Proceso de Creación</h3>
          ...
          <p className="text-center text-gray-600 dark:text-gray-400">
            ¿Interesado en nuestro proceso? <button onClick={() => setIsModalOpen(true)} className="text-light-accent dark:text-brand-gold hover:underline">Contáctanos para una consulta</button>.
          </p>
        </section>
        ...
        <section className="bg-light-card dark:bg-gray-900 rounded-lg p-6 mb-12">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
            <select name="ubicacion" onChange={handleFilterChange} className="bg-light-bg dark:bg-gray-800 border border-light-subtle dark:border-gray-700 text-light-text dark:text-white rounded p-2">
              <option value="todas">Todas las Ubicaciones</option>
              <option value="Cancún">Cancún</option>
              <option value="Polanco">Polanco</option>
              <option value="San Miguel de Allende">San Miguel de Allende</option>
            </select>
            <select name="precio" onChange={handleFilterChange} className="bg-light-bg dark:bg-gray-800 border border-light-subtle dark:border-gray-700 text-light-text dark:text-white rounded p-2">
            ...
            <select name="tipo" onChange={handleFilterChange} className="bg-light-bg dark:bg-gray-800 border border-light-subtle dark:border-gray-700 text-light-text dark:text-white rounded p-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredPropiedades.length > 0 ? (
              filteredPropiedades.map((prop) => {
                ...
                return (
                  <Link to={`/inmobiliaria/${prop.id}`} key={prop.id} className="block group">
                    <div className="relative bg-light-card dark:bg-gray-800 border border-light-subtle dark:border-gray-700 rounded-lg overflow-hidden shadow-lg h-full transition-all duration-300 ease-in-out group-hover:shadow-2xl group-hover:dark:shadow-brand-gold/20 group-hover:-translate-y-2">
                      ...
                      <button onClick={(e) => toggleFavorito(e, prop.id)} className="absolute top-4 right-4 bg-gray-900/50 p-2 rounded-full text-white hover:bg-gray-900/75 transition-colors z-10">
