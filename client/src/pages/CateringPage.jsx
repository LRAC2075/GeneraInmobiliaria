...
export default function CateringPage() {
  ...
  const services = [
    {
      icon: <Heart className="w-10 h-10 text-accent-600 dark:text-accent-500" />,
      title: 'Bodas de Lujo',
      description: 'Diseñamos experiencias culinarias que celebran el amor, con atención a cada detalle para tu día especial.',
    },
    {
      icon: <Briefcase className="w-10 h-10 text-accent-600 dark:text-accent-500" />,
    ...
    {
      icon: <GlassWater className="w-10 h-10 text-accent-600 dark:text-accent-500" />,
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12">
              {services.map((service, index) => (
                <AnimatedSection key={index} className="delay-[200ms]">
                  <div className="bg-neutral-100 dark:bg-neutral-800 p-8 rounded-lg text-center transform hover:-translate-y-2 transition-transform duration-300 shadow-sm hover:shadow-xl">
                    <div className="inline-block p-4 bg-accent-100 dark:bg-accent-900/50 rounded-full mb-4">
                      {service.icon}
                    </div>
                    <h3 className="text-xl font-bold text-primary-800 dark:text-neutral-100 mb-2">{service.title}</h3>
        ...
        <section className="py-20 px-4">
          <div className="container mx-auto">
            ...
            <AnimatedSection className="delay-[200ms] max-w-3xl mx-auto">
                ...
                {Object.keys(menus).map((key) => (
                  <button
                    ...
                    className={`px-4 py-3 text-sm md:text-base font-medium transition-colors duration-300 ${
                      activeTab === key
                        ? 'border-b-2 border-accent-600 text-accent-600 dark:text-accent-500'
                        : 'text-neutral-500 hover:text-primary-800 dark:text-neutral-400 dark:hover:text-neutral-100'
                    }`}
                  >
                    {menus[key].title}
                  </button>
              ...
              <div className="bg-neutral-100 dark:bg-neutral-800 p-8 rounded-lg shadow-inner">
                <h4 className="flex items-center gap-3 text-2xl font-bold text-primary-800 dark:text-neutral-100 mb-6">
                  <span className="text-accent-600 dark:text-accent-500">{menus[activeTab].icon}</span>
                  {menus[activeTab].title}
                </h4>
                <ul className="space-y-4">
                  {menus[activeTab].items.map((item, index) => (
                    <li key={index} className="flex items-start">
                      <UtensilsCrossed className="w-5 h-5 text-accent-600 dark:text-accent-500 mr-3 mt-1 flex-shrink-0" />
